...
namespace hpx { namespace threads { namespace detail
{
    ...
    defined(HPX_HAVE_THREAD_IDLE_RATES)
        ...
        boost::barrier startup(static_cast<unsigned>(num_threads+1));
        try {
    ...
    (defined(HPX_HAVE_MAX_CPU_COUNT) && HPX_HAVE_MAX_CPU_COUNT <= 64)
                ...
                threads_.push_back(new boost::thread(
                        util::bind(&thread_pool::thread_func, this, thread_num,
                            boost::ref(topology_), boost::ref(startup))
                    ));
            ...
            // trigger the barrier
            while (num_threads-- != 0 && !startup.wait())
                ;
        ...
        if (!threads_.empty()) {
            ...
            if (blocking) {
                for (std::size_t i = 0; i != threads_.size(); ++i)
                {
                    ...
                    sched_.Scheduler::do_some_work(i);
